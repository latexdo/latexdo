...
const defaultAiSourceRoot = path.resolve(projectRoot, "..", "ai.latexdo.org");
const aiSourceRoot = process.env.LATEXDO_AI_SOURCE_PATH
  ? path.resolve(projectRoot, process.env.LATEXDO_AI_SOURCE_PATH)
  : defaultAiSourceRoot;
const syncManifestPath = path.join(aiSourceRoot, "latexdo-sync.json");
...
async function syncFile(entry) {
  const source = path.join(aiSourceRoot, entry.from);
  const target = path.join(projectRoot, entry.to);
  await mkdir(path.dirname(target), { recursive: true });
  await writeFile(target, await readFile(source));
}

async function syncStyleFragment(entry) {
  const source = path.join(aiSourceRoot, entry.from);
  const target = path.join(projectRoot, entry.to);
...
async function syncRuntimeSource() {
  if (!(await fileExists(syncManifestPath))) {
    const message = `No AI source manifest found at ${syncManifestPath}`;
    if (
      process.env.LATEXDO_AI_SOURCE_REQUIRED === "1" ||
      process.env.LATEXDO_AI_SOURCE_PATH
    ) {
      throw new Error(message);
    }
    console.warn(`[ai-sync] ${message}; keeping checked-in AI source.`);
    return false;
  }
  ...
  for (const entry of manifest.styleFragments) {
    await syncStyleFragment(entry);
  }
  console.log(
    `[ai-sync] Synced ${manifest.files.length} AI files and ${manifest.styleFragments.length} style fragment(s) from ${path.relative(projectRoot, aiSourceRoot)}.`,
  );
  return true;
}
...
async function loadCatalog() {
  ...
  const configuredPath = process.env.LATEXDO_AI_CATALOG_PATH
    ...
    : null;
  const catalogPath =
    configuredPath ?? path.join(aiSourceRoot, "catalog/latexdo-ai-catalog.v1.json");
...
// Do not edit manually; edit the AI source repo and run npm run ai:sync.
