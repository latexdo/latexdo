...
describe("cloud API request policies", () => {
  ...
  it("retries a transient GET response with a bounded policy", async () => {
    ...
    expect(headers.get("x-latexdo-session")).toBe("session-1");
    expect(headers.get("x-latexdo-client")).toBe("client-1");
  });
  ...
  it("creates a durable local identity when none is stored", async () => {
    window.localStorage.clear();
    const fetchMock = vi
      .fn<typeof fetch>()
    ...
    expect(first.get("x-latexdo-session")).toMatch(/^session-/);
    expect(first.get("x-latexdo-client")).toMatch(/^client-/);
    expect(second.get("x-latexdo-session")).toBe(first.get("x-latexdo-session"));
    expect(second.get("x-latexdo-client")).toBe(first.get("x-latexdo-client"));
  });

  it("opens a shared project through the share token route", async () => {
    ...
    const headers = new Headers(fetchMock.mock.calls[0][1]?.headers);
    expect(requestUrl.pathname).toBe("/api/shares/share-secret/open");
  ...
  it("imports a DOCX through the authenticated hosted gateway", async () => {
    ...
    expect(new Headers(init?.headers).get("x-latexdo-session")).toBe("session-1");
