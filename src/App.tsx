...
function loadPersistedAiChatTabs(): PersistedAiChatTabs {
  try {
    ...
    const activeId =
      ...
      chats.some((chat) => chat.id === raw.activeId)
        ? raw.activeId
        : chats[0].id;
...
function loadCustomLatexDoAiModels(): CustomLatexDoAiModelRecord[] {
  try {
    ...
    return Array.from(
      new Map(records.map((record) => [record.fileName, record])).values(),
    );
  } catch {
    return [];
  }
...
export default function App() {
  ...
  const [downloadedLatexDoAiModelFiles, setDownloadedLatexDoAiModelFiles] = useState<
    Set<string>
  >(() => new Set());
