...
describe("runCitationChecks", () => {
  it("returns empty when disabled", () => {
    expect(runCitationChecks("content", { ...defaultSettings, enabled: false })).toHaveLength(0);
  });
  ...
  describe("Missing citations", () => {
    it("detects missing citations in claim paragraphs", () => {
      const doc = makeDoc("Our proposed method achieves state-of-the-art results on all benchmarks.");
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("without supporting citations"))).toBe(true);
    });

    it("passes when claims have citations", () => {
      const doc = makeDoc("Our proposed method achieves state-of-the-art results \\cite{ref1}.");
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("without supporting citations"))).toBe(false);
    });

    it("skips when disabled", () => {
      const doc = makeDoc("Our proposed method is novel.");
      const result = runCitationChecks(doc, { ...defaultSettings, detectMissingCitations: false });
  ...
  describe("Unused bibliography entries", () => {
    it("detects unused bibitem", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{ref1} Author.\\end{thebibliography}");
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("Unused"))).toBe(true);
    });

    it("passes when all bibitems are cited", () => {
      const doc = makeDoc("See \\cite{ref1}.\\begin{thebibliography}\\bibitem{ref1} Author.\\end{thebibliography}");
      const result = runCitationChecks(doc, defaultSettings);
    ...
    it("skips when disabled", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{ref1} A.\\end{thebibliography}");
      const result = runCitationChecks(doc, { ...defaultSettings, detectUnusedEntries: false });
      expect(result.some((d) => d.message.includes("Unused"))).toBe(false);
    });
  });
  ...
  describe("Duplicate references", () => {
    it("detects duplicate bibitem keys", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{ref1} A.\\bibitem{ref1} B.\\end{thebibliography}");
      const result = runCitationChecks(doc, defaultSettings);
    ...
    it("detects similar citation keys", () => {
      ...
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("duplicate") || d.message.includes("similar"))).toBe(true);
    });

    it("skips when disabled", () => {
      const doc = makeDoc("\\cite{ref1}. \\cite{ref1}.");
      const result = runCitationChecks(doc, { ...defaultSettings, detectDuplicateReferences: false });
  ...
  describe("Broken links", () => {
    ...
    it("detects URL without scheme in url command", () => {
      ...
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("missing scheme") || d.message.includes("no scheme"))).toBe(true);
    });
    ...
    it("skips when disabled", () => {
      ...
      const result = runCitationChecks(doc, { ...defaultSettings, detectBrokenLinks: false });
  ...
  describe("Citation key suggestions", () => {
    it("suggests citations for claim sentences", () => {
      const doc = makeDoc("As demonstrated in previous work, this approach works well.");
      const result = runCitationChecks(doc, defaultSettings);
    ...
    it("skips when disabled", () => {
      ...
      const result = runCitationChecks(doc, { ...defaultSettings, suggestCitationKeys: false });
  ...
  describe("Old citations", () => {
    it("warns about old citations (pre-2020)", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{oldref} Author, 1999.\\end{thebibliography}");
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("Old citation") || d.message.includes("old"))).toBe(true);
    });

    it("passes recent citations", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{newref} Author, 2025.\\end{thebibliography}");
      const result = runCitationChecks(doc, defaultSettings);
      expect(result.some((d) => d.message.includes("Old citation") || d.message.includes("old") || d.message.includes("predates"))).toBe(false);
    });

    it("skips when disabled", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{old} Author, 1980.\\end{thebibliography}");
      const result = runCitationChecks(doc, { ...defaultSettings, warnOldCitations: false });
