...
import type { PdfComplianceSettings, CitationAssistantSettings, AcronymManagerSettings } from "../../types";
...
function doc(b: string): string { return "\\documentclass{article}\n\\begin{document}\n" + b + "\n\\end{document}\n"; }
...
describe("Section citations", () => {
  const cases: [string, string, number][] = [
    ...
    ["3 sections 2 cites", "\\section{A}\\cite{r1}B\\section{C}\\cite{r2}D\\section{E}F", 1],
    ["3 sections 3 cites", "\\section{A}\\cite{r1}B\\section{C}\\cite{r2}D\\section{E}\\cite{r3}F", 0],
  ];
  it.each(cases)("%s = %i issues", (desc, body, expected) => {
    expect(runPdfComplianceChecks(doc(body), "", P).filter((d) => d.message.includes("no citations")).length).toBe(expected);
  });
});
...
describe("Figure references", () => {
  const cases: [string, string, boolean][] = [
    ...
    ["1 figure label no ref", "\\begin{figure}\\label{fig:a}\\caption{A}\\end{figure}", true],
    ["1 figure with ref", "\\begin{figure}\\label{fig:a}\\caption{A}\\end{figure} See \\ref{fig:a}.", false],
    ["2 figs unreffed", "\\begin{figure}\\caption{A}\\end{figure}\\begin{figure}\\caption{B}\\end{figure}", false],
    ["2 figs 1 reffed", "\\begin{figure}\\label{fig:a}\\caption{A}\\end{figure}\\begin{figure}\\label{fig:b}\\caption{B}\\end{figure} See \\ref{fig:a}.", true],
    ["2 figs both reffed", "\\begin{figure}\\label{fig:a}\\caption{A}\\end{figure}\\begin{figure}\\label{fig:b}\\caption{B}\\end{figure} See \\ref{fig:a} and \\ref{fig:b}.", false],
  ];
  it.each(cases)("%s → issues=%s", (desc, body, expected) => {
    expect(runPdfComplianceChecks(doc(body), "", P).some((d) => d.message.includes("never referenced"))).toBe(expected);
  });
});

// ── Page count boundary ─────────────────────────────────────────────────
describe("Page count", () => {
  const cases: [number, boolean][] = [[0,false],[1,false],[7,false],[8,false],[9,true],[10,true],[100,true]];
  it.each(cases)("pages=%i exceeds=%s", (pages, exceeds) => {
    expect(runPdfComplianceChecks(doc("Hi."), `Output written on paper.pdf (${pages} pages).`, P).some((d) => d.detail?.includes("exceed"))).toBe(exceeds);
  });
});

// ── Abstract word count boundary ─────────────────────────────────────────
describe("Abstract word count", () => {
  const cases: [number, boolean][] = [[0,false],[1,false],[100,false],[200,false],[250,false],[251,true],[300,true],[500,true]];
  it.each(cases)("words=%i exceeds=%s", (words, exceeds) => {
    const body = Array(words).fill("word").join(" ");
    expect(runPdfComplianceChecks(doc("\\begin{abstract}" + body + "\\end{abstract}"), "", P).some((d) => d.detail?.includes("exceed"))).toBe(exceeds);
  });
});

// ── Custom max page ──────────────────────────────────────────────────────
describe("Custom max page", () => {
  const cases: [number, number, boolean][] = [[5,4,true],[5,5,false],[5,6,false],[10,8,true],[10,10,false],[10,12,false],[0,8,false],[1,1,false],[100,100,false],[101,100,true]];
  it.each(cases)("pages=%i max=%i exceeds=%s", (pages, max, exceeds) => {
    expect(runPdfComplianceChecks(doc("Hi."), `Output written on paper.pdf (${pages} pages).`, { ...P, maxPages: max }).some((d) => d.detail?.includes("exceed"))).toBe(exceeds);
  });
});
...
describe("Acronym detection boundary", () => {
  ...
  it.each(cases)("%s", (desc, text, shouldFlag) => {
    expect(runAcronymChecks(doc(text), A).filter((d) => d.message.includes("used without definition")).length > 0).toBe(shouldFlag);
  });
});
...
describe("Acronym definition vs usage", () => {
  ...
  it.each(cases)("%s", (desc, text, hasUnused) => {
    expect(runAcronymChecks(doc(text), A).filter((d) => d.message.includes("never used again")).length > 0).toBe(hasUnused);
  });
});
