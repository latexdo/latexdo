...
const structureCases = [
  "", " ", "\n\n\n", "\\section{}", "\\section{A}\\label{}",
  "\\section{A}\\section{B}", "\\section{A}Content.\\section{B}Data.\\section{C}Done.",
  "\\section*{A}\\section*{B}", "\\section{A}\\label{sec:a}Content. See \\ref{sec:a}.",
  ...
  "\\begin{abstract}Test\\end{abstract}", "\\begin{abstract}\\end{abstract}",
  ...
  "\\section{Intro}We use \\gls{cpu}.", "\\section{A}\\begin{equation}a=b\\end{equation}",
...
const acronymCases = [
  "", "ABC XYZ DEF", "a b c d e f g", "CNN (CNN) then CNN again.",
  "ANN (ANN) used again.", "The LSTM RNN CNN GAN models.",
  "ABC (Apple) XYZ (Xerox) ABC XYZ.", "CPU GPU RAM SSD HDD.",
  "API SDK CLI GUI IDE.", "HTML CSS JS TS JSON XML.",
  "HTTP HTTPS FTP SSH TCP IP.", "SQL NoSQL ACID BASE CAP.",
  "PDF XML CSV JSON YAML.", "ARM x86 RISC CISC MIPS.",
  "USA UK UN NATO WHO.", "NASA ESA JAXA CNSA ISRO.",
  "AI (AI) ML (ML) DL (DL) all used.", "NLP (NLP) CV (CV) used.",
  "NaN null undefined.", "The quick FOX jumps over the lazy DOG.",
  "ReLU (ReLU) is used.", "SGD Adam optimizers.",
  "3D CNN model.", "CNN RNN LSTM GAN VAE BERT GPT ReLU SGD Adam.",
];
...
const citationCases = [
  "", "\\cite{r1}", "\\cite{r1,r2,r3}", "\\cite[see][p.5]{r1}",
  "See \\cite{r1} and \\cite{r2}.", "\\citep{r1} \\citet{r2}",
  "\\parencite{r1} \\textcite{r2}", "\\autocite{r1} \\footcite{r2}",
  "\\nocite{*}", "~\\cite{r1}",
  "\\label{sec:a} See \\ref{sec:a}.", "\\label{fig:x} See \\ref{fig:x}.",
  "\\label{tab:x} See \\ref{tab:x}.", "\\label{eq:x} See \\ref{eq:x}.",
  ...
  "\\cites{r1}{r2}{r3}", "\\cite[cf.][]{r1} \\cite[e.g.][]{r2}",
  "\\parencite{r1} \\footcite{r2} \\textcite{r3} \\smartcite{r4}",
  "\\citeauthor{r1} \\citeyear{r2}", "\\cite{r1,r2,r3,r4,r5,r6,r7,r8}",
];
...
const reproCases = [
  "", "\\url{https://github.com/user/repo}", "\\href{https://github.com}{GH}",
  "Dataset at \\url{https://zenodo.org/12345}", "MIT License",
  "Apache 2.0", "Creative Commons 4.0",
  "Learning rate 0.001, batch 64", "Adam optimizer",
  "Dropout 0.5, weight decay 1e-4", "NVIDIA A100 GPU",
  "Training time: 2 hours", "Random seed: 42",
  "numpy.random.seed(42)", "torch.manual_seed(42)",
  "Accuracy 95%, F1 0.93", "MAE 0.05, RMSE 0.1",
  "Seed 42. GPU. lr=0.001.", "MIT. Adam. A100. Seed 42. Acc 95%.",
  "Python 3.9, PyTorch 2.0", "Grid search over [1e-5, 1e-3]",
  "4x NVIDIA A100 80GB", "accuracy (95.2%), F1 (0.93), AUROC (0.97)",
];
...
// ── Notation: edge cases ─────────────────────────────────────────────────
const notationCases = [
  "", "$a+b=c$", "$\\alpha\\beta\\gamma$", "$\\theta\\lambda\\mu$",
  "$\\Phi\\Psi\\Omega$", "$x_i$ and $x_{ij}$", "\\[ \\sum_{i=1}^n x_i \\]",
  "$$ \\int_a^b f(x) dx $$", "$\\mathbb{R}^n$ and $\\mathcal{L}$",
  "\\newcommand{\\loss}{\\mathcal{L}} $\\loss$",
  "$\\hat{y}$ $\\tilde{x}$ $\\bar{z}$", "$\\dot{a}$ $\\ddot{b}$ $\\vec{v}$",
  "$\\nabla f$ $\\partial f/\\partial x$", "$\\cup\\cap\\subset\\supseteq$",
  "$\\in\\notin\\forall\\exists$", "$\\to\\rightarrow\\leftarrow\\mapsto$",
  "$\\approx\\sim\\cong\\equiv\\propto$", "$\\oplus\\otimes\\odot$",
  "$\\frac{a}{b} \\sqrt{x}$", "$\\binom{n}{k}$",
  "\\begin{equation}E=mc^2\\end{equation}", "\\begin{align}a&=b\\end{align}",
...
const confCases: [string, string][] = [
  ...
  ["\\documentclass{article}\\begin{document}" + Array(400).fill("w").join(" ") + "\\end{document}", ""],
  ["\\documentclass{article}\\begin{document}\\bibliographystyle{plain}\\bibliography{r}\\end{document}", ""],
  ["\\documentclass{article}\\begin{document}\\section{Intro}Hi.\\end{document}", "Output written on paper.pdf (5 pages)."],
...
const errorCases: [string, string][] = [
  ["", ""],
  ["\\documentclass{article}\\begin{document}Hi.\\end{document}", "Output written on paper.pdf."],
