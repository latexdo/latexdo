...
describe("Common words NOT flagged — parameterized (acronym manager)", () => {
  it.each(commonWords)("'%s' should not be flagged as undefined acronym", (word) => {
    const content = doc(
      `The ${word} is a common English word used in academic writing.`,
    );
...
const definedAcronyms = [
  ...
  {
    ac: "AAAI",
    def: "Association for the Advancement of Artificial Intelligence (AAAI)",
  },
...
describe("Defined acronyms NOT flagged — parameterized", () => {
  it.each(definedAcronyms)(
    "'$ac' with definition should not be flagged as undefined",
    ({ ac, def }) => {
...
describe("Lowercase variants not flagged — parameterized", () => {
  it.each(caseSensitiveAcros)(
    "'$ac' lowercase '$text' not flagged when defined",
    ({ ac, text }) => {
