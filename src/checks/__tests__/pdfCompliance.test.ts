...
describe("runPdfComplianceChecks", () => {
  it("returns empty when disabled", () => {
    const result = runPdfComplianceChecks("content", "output", { ...defaultSettings, enabled: false });
  ...
  describe("Page count", () => {
    it("warns when PDF exceeds page limit", () => {
      ...
      const result = runPdfComplianceChecks(content, output, defaultSettings);
      expect(result.some((d) => d.message.includes("exceed") || d.message.includes("pages"))).toBe(true);
    });
    ...
    it("skips page check when disabled", () => {
      const content = makeDoc("Hello.");
      const output = "Output written on paper.pdf (10 pages, 12345 bytes).";
      const result = runPdfComplianceChecks(content, output, { ...defaultSettings, checkPageCount: false });
      expect(result.some((d) => d.message.toLowerCase().includes("page") && d.message.toLowerCase().includes("limit"))).toBe(false);
    });
  });

  describe("Unreferenced figures", () => {
    it("detects unreferenced figure label", () => {
      const doc = makeDoc("\\begin{figure}\\label{fig:myplot}\\caption{A plot}\\end{figure}");
    ...
    it("passes when figure is referenced", () => {
      const doc = makeDoc("\\begin{figure}\\label{fig:myplot}\\caption{A plot}\\end{figure} As shown in \\ref{fig:myplot}.");
      const result = runPdfComplianceChecks(doc, "", defaultSettings);
    ...
    it("skips figure check when disabled", () => {
      ...
      const result = runPdfComplianceChecks(doc, "", { ...defaultSettings, checkUnreferencedFigures: false });
      expect(result.some((d) => d.message.includes("never referenced"))).toBe(false);
    });
  });
  ...
  describe("Uncited citations", () => {
    it("detects uncited bibliography entry", () => {
      const doc = makeDoc("\\begin{thebibliography}\\bibitem{ref1} Author, 2024.\\end{thebibliography}");
      const result = runPdfComplianceChecks(doc, "", defaultSettings);
      expect(result.some((d) => d.message.includes("never cited"))).toBe(true);
    });

    it("passes when all entries are cited", () => {
      const doc = makeDoc("See \\cite{ref1}.\\begin{thebibliography}\\bibitem{ref1} Author, 2024.\\end{thebibliography}");
      const result = runPdfComplianceChecks(doc, "", defaultSettings);
    ...
    it("skips citation check when disabled", () => {
      ...
      const result = runPdfComplianceChecks(doc, "", { ...defaultSettings, checkUncitedCitations: false });
      expect(result.some((d) => d.message.includes("never cited"))).toBe(false);
    });
  });
  ...
  describe("Sections with no citations", () => {
    ...
    it("skips section citation check when disabled", () => {
      const doc = makeDoc("\\section{Method}\nNo cites.");
      const result = runPdfComplianceChecks(doc, "", { ...defaultSettings, checkSectionsWithNoCitations: false });
      expect(result.some((d) => d.message.includes("no citations"))).toBe(false);
    });
  });
  ...
  describe("Type 3 fonts", () => {
    it("detects Type 3 fonts in output", () => {
      const result = runPdfComplianceChecks("Content", "This PDF uses Type 3 fonts.", defaultSettings);
      expect(result.some((d) => d.message.includes("Type 3"))).toBe(true);
    });

    it("skips font check when no output", () => {
      const result = runPdfComplianceChecks("Content", "", defaultSettings);
      expect(result.some((d) => d.message.includes("font") || d.message.includes("Type 3"))).toBe(false);
    });

    it("skips font check when disabled", () => {
      const result = runPdfComplianceChecks("Content", "Type 3 fonts.", { ...defaultSettings, checkType3Fonts: false });
  ...
  describe("Abstract word count", () => {
    it("warns when abstract exceeds word limit", () => {
      ...
      const doc = "\\begin{document}\\begin{abstract}" + words + "\\end{abstract}\\end{document}";
      const result = runPdfComplianceChecks(doc, "", defaultSettings);
    ...
    it("skips abstract word count when disabled", () => {
      const words = Array(300).fill("word").join(" ");
      const doc = "\\begin{document}\\begin{abstract}" + words + "\\end{abstract}\\end{document}";
      const result = runPdfComplianceChecks(doc, "", { ...defaultSettings, checkAbstractWordCount: false });
  ...
  describe("Edge cases", () => {
    ...
    it("handles compile output without page info", () => {
      const doc = makeDoc("Hello.");
      const result = runPdfComplianceChecks(doc, "Some compile log without page info.", defaultSettings);
      expect(Array.isArray(result)).toBe(true);
    });
